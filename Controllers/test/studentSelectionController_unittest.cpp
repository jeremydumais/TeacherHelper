...
{
	StudentSelectionController controller(FakeDatabaseController(), make_unique<FakeStudentController>());
    ASSERT_TRUE(controller.isStudentInFilter("TEST", Student("Joe", "Blow", "Test")));
}
...
TEST(StudentSelectionController_findStudent, StandardCall_ReturnSuccess)
{
	...
	StudentSelectionController controller(FakeDatabaseController(), move(studentController));
